...
const itemRouter = (itemController) => {
  router.get("/", async (req, res) => {
    ...
    const { itemsNumber, pagesNumber, items } = await itemController.getAllItems(page, limit, filters, search);
    res.status(200).send({
  ...
  router.post("/", auth, upload.fields([{ name: "thumbnail", maxCount: 1 }]), async (req, res) => {
  ...
  router.patch("/:id", auth, upload.fields([{ name: "thumbnail", maxCount: 1 }]), async (req, res) => {
    ...
    const updatedItem = await itemController.updateItem(itemId, userId, itemData);
